...
//------------------------------------------------------
{
	...
	private int			  m_iFriendlySkyX1;
	private int			  m_iFriendlySkyX2;
	...
	//------------------------------------------------------
	{
		...
		if(m_oCanvas.m_bSpeedOptimized == true)m_iNumberOfMenToCreate = 4;
...
*/
	...
	//------------------------------------------------------
	{
		...
		if(m_oCanvas.m_bSpeedOptimized == true)
		{
			...
			for(int x=0; x < 5;x++)
			{
				if(m_iObjectType[x] == 0)continue;
				if(m_iObjectType[x] == 3)continue;
				if(m_iObjectType[x] == 5)continue;
				if(m_iObjectType[x] == 6)continue;
...
*/
				for(int i=0;i < 12;i+=2)
				{
					...
					else if(m_iFriendlySoldierFight[i] == 0)
					{
						...
						if(m_bFriendlySoldierDead[i] == false)m_iFriendlySkyX2 = m_iFriendlySoldierX[i];
					}
				}
		...
		if(keyState == Canvas.LEFT || keyState == Canvas.RIGHT || keyState == Canvas.UP || keyState == Canvas.DOWN ||
				iKey == Canvas.KEY_NUM2 || iKey == Canvas.KEY_NUM4 || iKey == Canvas.KEY_NUM6 || iKey == Canvas.KEY_NUM8)m_iFriendlyFireFlash = 0;
...
//				g.drawImage(m_oCanvas.m_oImageArray[1],(x*20)-90,(y*20),Graphics.TOP | Graphics.LEFT);
		...
		if(m_iFriendlyAnim < 5)
		{
			for(i = 0;i < m_iNumberOfMenToCreate;i++)
			{
				if(!m_bFriendlySoldierDead[i])
				{
					...
					else if(m_iFriendlyFireFlash > 4)g.drawImage(m_oCanvas.m_oImageArray[3],m_iFriendlySoldierX[i]-40,m_iFriendlySoldierY[i]-21,Graphics.TOP | Graphics.LEFT);
	...
	//------------------------------------------------------
	{
		...
		if(m_iFriendlyFireFlash > 0)
		{
			for(i = 0;i < m_iNumberOfMenToCreate;i++)
			{
				if(m_bFriendlySoldierDead[i] || m_iFriendlySoldierFight[i] != 0)continue;
				if((i % 2) == 0){if(m_bFriendlySoldierDead[i+1] == false)continue;}
				
				int xCoordinate;
				if((i % 2) == 0)xCoordinate = m_iFriendlySkyX2;
				else 			xCoordinate = m_iFriendlySkyX1;

				g.setClip(xCoordinate+3,m_iFriendlyBulletY[i/2]-5-6,20,20);
				if(m_iFriendlyFireFlash >  9 && m_iFriendlyFireFlash <= 10)g.drawImage(m_oCanvas.m_oImageArray[3],xCoordinate+3-63,m_iFriendlyBulletY[i/2]-5-6,Graphics.TOP | Graphics.LEFT);
				if(m_iFriendlyFireFlash >  6 && m_iFriendlyFireFlash <=  9)g.drawImage(m_oCanvas.m_oImageArray[3],xCoordinate+3-42,m_iFriendlyBulletY[i/2]-5-6,Graphics.TOP | Graphics.LEFT);
				if(m_iFriendlyFireFlash >  3 && m_iFriendlyFireFlash <=  6)g.drawImage(m_oCanvas.m_oImageArray[3],xCoordinate+3-21,m_iFriendlyBulletY[i/2]-5-6,Graphics.TOP | Graphics.LEFT);
				if(m_iFriendlyFireFlash >  0 && m_iFriendlyFireFlash <=  3)g.drawImage(m_oCanvas.m_oImageArray[3],xCoordinate+3   ,m_iFriendlyBulletY[i/2]-5-6,Graphics.TOP | Graphics.LEFT);
			}
		...
		if(m_iEnemyFireFlash > 0)
		{
			for(i = 0;i < m_iNumberOfMenToCreate;i++)
			{
				...
				if((i % 2) == 1){if(m_bEnemySoldierDead[i-1] == false)continue;}			
	...
	//------------------------------------------------------
	{
		...
		|| m_oCanvas.m_oStateGame.m_iEnemyShipShipType[m_oCanvas.m_oStateGame.m_iShipNr] == 18  // Pirate ship
		|| m_oCanvas.m_oStateGame.m_iEventOld == 8  // Pirate event
		|| m_oCanvas.m_oStateGame.m_iEventOld == 4) // Pirate event
		{
			AIAvoidShoot();
			AIAnalyzeShoot();
			AIAnalyzeMove();	
				...
				|| m_oCanvas.m_oStateGame.m_iEnemyShipShipType[m_oCanvas.m_oStateGame.m_iShipNr] == 15)
		{
			...
			{
				m_iDifficult = 15;
				if(m_oCanvas.m_oRand.nextInt()%2 == 0)AIAnalyzeShoot();
			}
			//if(m_iDifficult > 5)AIAvoidShoot();
			AIAnalyzeMove();
				...
				|| m_oCanvas.m_oStateGame.m_iEnemyShipShipType[m_oCanvas.m_oStateGame.m_iShipNr] == 13)
		{
			...
			{
				m_iDifficult = 30;
				if(m_oCanvas.m_oRand.nextInt()%4 == 0)AIAnalyzeShoot();
				
			}
			if(m_iDifficult >= 25)AIAvoidShoot();
			if(m_iDifficult <= 20 && m_iDifficult >= 5)AIAnalyzeMove();	
		}
